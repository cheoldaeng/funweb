	...
	<div id="wrap">
		...
		<article>
			...
			<table id="notice">
				...
				<tr>
					<th scope="col">작성일</th>
					<td style="text-align: left;"><javatime:format
				...
				<tr>
					...
					<c:if test="${ not empty attachList }">
							<c:forEach var="attach" items="${ attachList }">	<%-- ${ not empty boardVo.attachList } --%>
								<c:choose>
			...
			<div id="table_search">
				...
				<button type="button"
					onclick="location.href='/board/filereplyWriteForm?reRef=${ boardVo.reRef }&reLev=${ boardVo.reLev }&reSeq=${ boardVo.reSeq }&pageNum=${ pageNum }'">답글쓰기</button>
				<button type="button"
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
	      <div class="form-group">
				<label for="reply">댓글내용</label> <!-- for='' 해당 id로 연결 -->
				<input type="text" class="form-control" placeholder="댓글내용" name="reply" id="reply">
		  </div>
		  <div class="form-group">
				<label for="replyer">댓글 작성자</label> 
				<input type="text" class="form-control disabled" placeholder="댓글 작성자" name="replyer" id="replyer">
		  </div>
		  <div class="form-group">
...
// 함수 정의용
	...
	// 댓글목록 데이터 가공하여 화면에 추가하기 - 리스트 아이템화
	...
	function showList(pageNum) {
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (pageNum == -1){
					var endPage = Math.ceil(result.replyCnt / 10.0);
					showList(endPage);
					return;
				}
	...
	showList(pageNum); // -1은 댓글 마지막 페이지 기준으로 댓글목록 보이라는 의미. //1 : 최신페이지
...
//이벤트 연결용
	...
	btnModalRegister.on('click', function () {
		...
		var str = JSON.stringify(reply); //서버에 전송해야할 문자열
